...
#
# Example:
# ./release-against-charts.sh "${GITHUB_WORKSPACE}" "v0.23.0-rc.0"
...
# Get the turtles version (eg: 0.23.0-rc.0)
...
if [ "$is_new_minor" = "true" ]; then
    ...
    COMMIT_MSG="Bump turtles to ${NEW_CHART_VERSION}+up${NEW_TURTLES_VERSION_SHORT} (chart major bump)"
elif [ "$is_prev_rc" = "false" ]; then
    NEW_CHART_VERSION=$(bump_patch "$PREV_CHART_VERSION")
    COMMIT_MSG="Bump turtles to ${NEW_CHART_VERSION}+up${NEW_TURTLES_VERSION_SHORT} (chart patch bump)"
else
    NEW_CHART_VERSION=$PREV_CHART_VERSION
    COMMIT_MSG="Bump turtles to ${NEW_CHART_VERSION}+up${NEW_TURTLES_VERSION_SHORT} (no chart bump)"
fi
